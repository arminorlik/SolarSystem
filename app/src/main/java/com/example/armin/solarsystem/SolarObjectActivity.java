...
public class SolarObjectActivity extends AppCompatActivity implements SolarObjectAdapter.SolarObjectClickedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean hasMovie = !TextUtils.isEmpty(solarObject.getVideo()); //sprawdzenie czy obiekt ma film;
        if (hasMovie) {
            ...
            CoordinatorLayout.LayoutParams lp = (CoordinatorLayout.LayoutParams) fab.getLayoutParams(); //wymuszenie aby coordinator layout
            lp.setAnchorId(View.NO_ID);                                                                 //nie zarządzał fab (bez tego visible
                                                                                                        //zawsze będzie na true
    ...
    private void showYouTubeVideo(String video) {
        ...
        try { //opakowanie try catchem na wypadek gdyby urządzenie nie posiadało aplikacji Youtube,
            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("vnd.youtube:" + video)); //uruchomienie aplikacji Youtube do uruchomienia filmu
            startActivity(intent);
        } catch (ActivityNotFoundException e) { //uruchomienie wyświetlania filmu w przeglądarce,
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.action_settings_set_as_wallpaper) {
    ...
    private void setAsWallpaper(String image) {
        WallpaperManager wallpaperManager = WallpaperManager.getInstance(this); //użycie wbdowanej metody do obsługi tapet
        try {
